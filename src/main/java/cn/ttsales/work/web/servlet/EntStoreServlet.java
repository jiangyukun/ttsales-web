 ...
 */
public class EntStoreServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;
	private Logger log = Logger.getLogger(EntStoreServlet.class);
	private String token = BundleUtil.getProperty(SASConstants.RESOURCE_WEIXIN,
			"wx.corp.agentStore.token");
	private String encodingAesKey = BundleUtil.getProperty(
			SASConstants.RESOURCE_WEIXIN, "wx.corp.agentStore.aesKey");
	...
	protected void doGet(HttpServletRequest request,
		...
		log.info("EntStoreServlet doGet");
		...
		if (result.getCode() == 0) {// 消息解密正确
			...
			case EVENT: {
				...
				case CLICK: {
					...
					if ("SIGN_IN_QCCODE".equals(eventKey)) {// 获取本店签到二维码
						log.info(eventKey + " start---------------------");
						WebApplicationContext ctx = WebApplicationContextUtils
								.getWebApplicationContext(this
										.getServletContext());
						EntStoreResponseService entStoreResponseService = (EntStoreResponseService) ctx
								.getBean("entStoreResponseService");
						entStoreResponseService.sendSignInQCCode(agentId,
								fromUserName, request.getServerName(),
								request.getContextPath(), this.getServletContext().getRealPath(""));
						out.write("");
						log.info(eventKey + " end------------------------");
					}
					break;
				}
				case LOCATION:
