var schemeCommon = function() {
	...
	function showSubDlg(){ 
		if(global.pTransmitId == "0"){
 		...
 		var openId = $.cookie("openId_501002"); 
	...
	function shareBtn(){
		...
		if(global.pTransmitId == "0"){
			location.href = $.sas.common.getRootPath()+'/qy/lottery/outerIn.do?schemeId=501002' ;
			return;
		}  
 		var isShare = $.cookie("is_share_scheme_501002"); 
 		var openId = $.cookie("openId_501002");
  		 if(isShare=='1' && openId == undefined){
 			var bckurl = $.base64Encode('/qy/lottery/outerIn.do?schemeId=501002');
			location.href = $.sas.common.getRootPath()+'/common/authorize.do?backurl='+ bckurl;
 			return;
 		}else if(isShare=='1'){
 			window.location.href = $.sas.common.getRootPath()+'/qy/lottery/outerIn.do?schemeId=501002';
 			return;
 		}else{
// 			$("[data-role='rtGuide']").rtGuide({});
//			$.fn.rtGuide.methods.show($('#rt_guide'));
//			$('#rt-guide-overlay').on('touchstart',function(){
//				$.fn.rtGuide.methods.hide($('#rt_guide'));
//			});
			return;
 		}
	}
	function shareSchemeSuccess(){
		if(global.pTransmitId == "0"){
			$.sas.util.setCookie("is_share_scheme_501002",'1',365);
			return ;  
		}
 		var isShare = $.cookie("is_share_scheme_501002"); 
		if(isShare==undefined||isShare=='0'){
			$('#dialog_content').text('成功获得一次抽奖机会！');
			$.fn.rtGuide.methods.hide($('#rt_guide')); 
			$('#shareSuccessDialog').rtDialog('show');
			$('.rt-dialog-cancel-button').on('touchstart',function(){
				$('#shareSuccessDialog').rtDialog('hide');
			});
			$('.rt-dialog-ok-button').on('touchstart',function(){
				$('#shareSuccessDialog').rtDialog('hide');
				var openId = $.cookie("openId_501002"); 
				if(openId==undefined||openId==''){
					var bckurl = $.base64Encode('/qy/lottery/outerIn.do?schemeId=501002');
					...
					location.href = $.sas.common.getRootPath()+'/qy/lottery/outerIn.do';
				}   
			});
 		}
		$.sas.util.setCookie("is_share_scheme_501002",'1',365);
...
//	}
	...
	return{
		...
		wantLottery:function(){
			alert('亲，来年再见哦~~更多红包等你拿~~恭祝新年快乐！');
		},
		saveWishRecord: function() {
			$.ajax({
				url: 'bus/popularize/saveWishRecord.do',
				data: {
					transmitId: global.pTransmitId,
					popularizeId : global.popularizeId
				}, 
				complete: function() {
					location.href = 'http://www.gmmc.com.cn/home';
				}
			});
